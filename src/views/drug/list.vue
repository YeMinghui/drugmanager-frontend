  ...
  <div class="app-container">
    <el-row class="filter-container" :gutter="5">
      ...
      <el-col :sm="5">
        ...
        <el-button @click="showDialog({opr: 'add', drug: {}})" type="primary">添加</el-button>
...
<!--          <el-button type="primary" size="mini" @click="$router.push({path: '/drug/info', params: {opr: 'edit', drug: row}})">-->
          <el-button type="text" size="mini" @click="showDialog({opr: 'edit', drug: row})">
            修改
          </el-button>
<!--          <el-button size="mini" type="danger" @click="$router.push({path: '/drug/info', params: {opr: 'delete', drug: row}})">-->
          <el-button size="mini" type="text" @click="showDialog({opr: 'delete', drug: row})">
            删除
          </el-button>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" fullscreen center close-on-press-escape>
      ...
      <span v-if="opr !== 'info'" slot="footer" class="dialog-footer">
        <el-button native-type="reset">清除</el-button>
        ...
        <el-button @click="deleteDrug" type="danger" v-if="opr==='delete'">删除</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
<script>
  ...
  import {deleteDrugs, listDrugs, putDrug, saveDrug, search} from "@/api/drug";
    ...
    methods: {
      ...
      deleteSelect: function() {
        if (this.multipleSelection.length === 0) {
          ...
          deleteDrugs(JSON.stringify(ids)).then(()=>{
      ...
      deleteDrug: function () {
        if (this.drug) {
          this.$confirm('此操作将永久删除, 是否继续?', '提示', {
            ...
            deleteDrugs({id: this.drug.id}).then(response=>{
              if (response.data) {
