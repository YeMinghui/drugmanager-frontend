<template>
  <div class="app-container">
    ...
    <el-table :data="tableData" border style="width: 100%;" @selection-change="handleSelectionChange" v-loading="table_loading">
      ...
      <el-table-column label="邮编" prop="postcode" sortable>
      </el-table-column>
...
<script>
  ...
  export default {
    ...
    data() {
      ...
      var rule_postcode = (rule, value, callback) => {
      }
      return {
        temp: {},
        manufacturers: [],
        ...
        rules: {
          ...
          postcode: [{required: true, validator: rule_postcode, trigger: 'blur'}],
        }
      }
    },
    created() {
    ...
    methods: {
      ...
      deleteSelect: function() {
        if (this.multipleSelection.length === 0) {
          ...
          deleteManufacturers(JSON.stringify(ids)).then(()=>{
      ...
      loadManufacturer: function() {
        listManufacturers({page: this.currentPage, limit: this.pageSize}).then(response => {
          ...
          this.total = total
          this.pages = pages
          this.tableData = this.Manufacturers
        })
      },
      hideDialog: function() {
      ...
      handleAdd: function () {
        if (this.temp) {
          saveManufacturer(this.temp).then(response => {
            if (response.data) {
      ...
      handleEdit: function () {
        if (this.temp) {
          putManufacturer(this.temp).then(response=>{
            if (response.data) {
      ...
      handleDelete: function () {
        if (this.temp) {
          this.$confirm('此操作将永久删除, 是否继续?', '提示', {
            ...
            deleteManufacturer({id: this.temp.id}).then(response=>{
              if (response.data) {
      ...
      handleFilter: function () {
        ...
        if (filter) {
          this.tableData = this.Manufacturers.filter(function (dataNews) {
